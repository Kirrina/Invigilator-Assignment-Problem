...
def save_solver_metrics(metrics: dict, output_dir: str = None):
    ...
    Cấu trúc metrics được lưu (đồng bộ model_builder.py + solver.py):
        status                  : 'Optimal' | 'Near-Optimal' | 'Feasible' | 'Infeasible'
        obj_value               : Giá trị hàm mục tiêu từ solver
        objective               : Alias của obj_value (dùng trong analysis.py)
        solve_time              : Thời gian giải (giây)
        gap / high / low        : Fairness gap = W_high - W_low
        slack_cap               : Tổng giá trị slack_cap (số vị trí thiếu nhân sự)
        slack_busy              : Tổng giá trị slack_busy (Binary → tổng val, không phải count)
        slack_qual              : Tổng giá trị slack_qual (Binary → tổng val, không phải count)
        fatigue_count           : Tổng số lần kích hoạt cờ yfatigue
        travel_count            : Tổng số lần kích hoạt cờ ypair
        M3_static_penalty       : P_ijr * x_ijr (campus + over/underqualification ×50)
        M4_fatigue_penalty      : 40*flag3 + 80*flag4 + 150*flag5 (cộng dồn per staff/day)
        M5_travel_penalty       : pair_penalties * ypair (travel + idle time)
        M1_total_quality        : M3 + M4 + M5
        _workload_dist          : {staff_id: số_ca} — phân phối khối lượng công việc
    """
    if output_dir is None:
        output_dir = _get_cache_dir()
...
def compute_baseline_metrics(file_excel: str, B_ij: dict, Campus_like_ik: dict,
    ...
    for _, row in df.iterrows():
        ...
        # M3 — Phạt tĩnh (campus + lãng phí/thiếu năng lực)
        # ĐỒNG BỘ VỚI model_builder.py — P_ijr:
        ...
        p_qual = (
            5.0  * max(0, staff_level - required_level) +   # overqualification
            50.0 * max(0, required_level - staff_level)     # underqualification
        )
    ...
    # ── M4: Fatigue Penalty ────────────────────────────────────────────────
    # ĐỒNG BỘ VỚI model_builder.py — 3 flag cộng dồn per (staff, day):
    #   daily_total <= 2 + yfatigue_3 + yfatigue_4 + yfatigue_5
    #   yfatigue_4 <= yfatigue_3,  yfatigue_5 <= yfatigue_4
    #   → cnt=3: chỉ flag_3=1       → penalty = 40
    #   → cnt=4: flag_3=flag_4=1    → penalty = 40 + 80  = 120
    #   → cnt≥5: flag_3=4=5=1       → penalty = 40+80+150 = 270
    penalty_fatigue = 0.0
    fatigue_details = {}
    for (i, d), cnt in staff_daily_count.items():
        fatigue_details[(i, d)] = cnt
        if cnt >= 3: penalty_fatigue += 40
        if cnt >= 4: penalty_fatigue += 80
        if cnt >= 5: penalty_fatigue += 150
...
def generate_dashboard_and_plots(baseline_metrics: dict, solver_metrics_raw: dict,
    ...
    # Ưu tiên dùng objective đã lưu trực tiếp từ solver (chính xác nhất)
    # Fallback: tính lại từ các thành phần nếu không có
    if 'objective' in s and s['objective'] is not None:
        solver_objective = float(s['objective'])
    else:
        solver_objective = float(s.get('obj_value',
